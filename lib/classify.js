...
const RULES = [
  ...
  // Prefixes are constrained on purpose: a bare ^[A-Z0-9]{8}$ also matched
  // model/batch codes (e.g. WD10EZEX), minting false "serials". `W\d`/`S\d`
  // exclude the WD/Seagate ST model prefixes (W followed by a letter).
  { kind: 'serial', re: /^(?:Z[A-Z0-9]{7}|W[0-9][A-Z0-9]{6}|S[0-9][A-Z0-9]{6}|BN[A-Z0-9]{6})$/, formats: ['Code39'] },
...
function parsePayload(text) {
  ...
  // real payloads are not rejected just because of line wrapping in the code.
  const norm = text.trim().replace(/\s+/g, ' ');
  if (!/^[A-Z]{2,5}:/.test(norm)) return null;
  const pairs = [...norm.matchAll(/([A-Z]{2,5}):([^\s]+)/g)];
  if (!pairs.length) return null;
  // the whole string must consist of KEY:value pairs separated by spaces
  if (pairs.map(m => m[0]).join(' ') !== norm) return null;
  return pairs.map(m => {
    const [, key, value] = m;
